window.addEventListener("load", function () {
  ...
  function askInterstitial(callback, runId) {
    if (runId !== submitRunId || FLOW[current] !== "submit" || !submitStarted) {
      return;
    }
  ...
  overlay.querySelectorAll("[data-answer]").forEach(function (btn) {
    btn.addEventListener("click", function () {
      if (FLOW[current] !== "submit" || !submitStarted) {
  ...
  function animateBar(row, done, runId) {
    ...
    function startTimer() {
      ...
      timer = setInterval(function () {
        ...
        if (!modalShown && value < 50) {
          value = Math.min(50, value + 4);
        } else {
  ...
  function resetSubmit() {
    ...
    delete answers.interstitial1;
    delete answers.interstitial2;
    delete answers.interstitial3;
