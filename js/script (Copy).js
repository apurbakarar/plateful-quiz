window.addEventListener("load", function () {
  ...
  function show(index) {
    ...
    if (currentStep === "submit" && !submitStarted) startSubmit();
  ...
  function askInterstitial(callback) {
  ...
  function startSubmit() {
    ...
    animateBar(rows[0], function () {
      animateBar(rows[1], function () {
        animateBar(rows[2], function () {
          overlay.classList.remove("open");
